...
import { readGlobalConfig } from '../packages/skillhub/src/config'
...
function mustGetToken() {
  const fromEnv = process.env.SKILLHUB_E2E_TOKEN?.trim() || process.env.SKILLHUB_E2E_TOKEN?.trim()
...
function getRegistry() {
  return (
    process.env.SKILLHUB_REGISTRY?.trim() ||
    process.env.SKILLHUB_REGISTRY?.trim() ||
    'https://skillhub.ai'
  )
}
...
describe('skillhub e2e', () => {
  it('prints CLI version via --cli-version', async () => {
    const result = spawnSync('bun', ['skillhub', '--cli-version'], {
      cwd: process.cwd(),
      encoding: 'utf8',
    })
  ...
  it('cli search does not error on multi-result responses', async () => {
    ...
    try {
      ...
      const result = spawnSync(
        ...
        {
          cwd: process.cwd(),
          env: { ...process.env, SKILLHUB_CONFIG_PATH: cfg.path, SKILLHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
        },
      )
  ...
  it('assumes a logged-in user (whoami succeeds)', async () => {
    ...
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
      throw new Error('Missing token. Set SKILLHUB_E2E_TOKEN or run: bun skillhub auth login')
    }
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['skillhub', 'whoami', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: { ...process.env, SKILLHUB_CONFIG_PATH: cfg.path, SKILLHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
  ...
  it('sync dry-run finds skills from special-agent.json roots', async () => {
    const registry = getRegistry()
    const site = getSite()
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'skillhub-e2e-special-agent-'))
    const stateDir = join(root, 'state')
    const configPath = join(root, 'special-agent.json')
    const workspace = join(root, 'special-agent-work')
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['skillhub', 'sync', '--dry-run', '--all', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: {
            ...process.env,
            SKILLHUB_CONFIG_PATH: cfg.path,
            SKILLHUB_DISABLE_TELEMETRY: '1',
  ...
  it('publishes, deletes, and undeletes a skill (logged-in)', async () => {
    ...
    try {
      ...
      const list = spawnSync(
        'bun',
        ['skillhub', 'list', '--site', site, '--registry', registry, '--workdir', installWorkdir],
        {
          cwd: process.cwd(),
          env: { ...process.env, SKILLHUB_CONFIG_PATH: cfg.path, SKILLHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
