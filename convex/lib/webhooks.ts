...
const DEFAULT_SITE_URL = 'https://skillhub.ai'
...
function buildDescription(event: WebhookEvent, skill: WebhookSkillPayload) {
  ...
  if (event === 'skill.highlighted') return 'Newly highlighted skill on SkillHub.'
  if (skill.version) return `New version v${skill.version} published on SkillHub.`
  return 'New skill published on SkillHub.'
}
