...
describe('skills', () => {
  it('extracts zip into directory and skips traversal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'skillhub-'))
    const zip = zipSync({
  ...
  it('writes and reads lockfile', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-work-'))
  ...
  it('returns empty lockfile on invalid json', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-work-bad-'))
    await mkdir(join(workdir, '.skillhub'), { recursive: true })
    await writeFile(join(workdir, '.skillhub', 'lock.json'), '{', 'utf8')
    const read = await readLockfile(workdir)
    expect(read).toEqual({ version: 1, skills: {} })
  })

  it('returns empty lockfile on schema mismatch', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-work-schema-'))
    await mkdir(join(workdir, '.skillhub'), { recursive: true })
    await writeFile(
      join(workdir, '.skillhub', 'lock.json'),
  ...
  it('skips dotfiles and node_modules when listing text files', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-files-'))
  ...
  it('respects .gitignore and .skillhubignore', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-ignore-'))
    await writeFile(join(workdir, '.gitignore'), 'ignored.md\n', 'utf8')
    await writeFile(join(workdir, '.skillhubignore'), 'private.md\n', 'utf8')
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('falls back to text/plain for unknown text extensions', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-env-'))
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('returns null for invalid skill origin metadata', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'skillhub-origin-'))
    expect(await readSkillOrigin(workdir)).toBeNull()

    await mkdir(join(workdir, '.skillhub'), { recursive: true })
    await writeFile(
      join(workdir, '.skillhub', 'origin.json'),
