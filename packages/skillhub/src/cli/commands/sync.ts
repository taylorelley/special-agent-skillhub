...
export async function cmdSync(opts: GlobalOpts, options: SyncOptions, inputAllowed: boolean) {
  ...
  if (!token) fail('Not logged in. Run: skillhub login')
  ...
  const primaryScan = await scanRootsWithLabels(combinedRoots, specialAgentRoots.labels)
  ...
  if (primaryScan.skills.length === 0) {
    ...
    if (fallbackScan.skills.length === 0)
      fail('No skills found (checked workdir and known Special Agent locations)')
    printSection(
