...
vi.mock('../../config.js', () => ({
  readGlobalConfig: vi.fn(async () => ({ registry: 'https://skillhub.ai', token: 'tkn' })),
}))

const mockGetRegistry = vi.fn(async () => 'https://skillhub.ai')
vi.mock('../registry.js', () => ({
...
const mockResolveSpecialAgentSkillRoots = vi.fn(
  async () =>
    ({
      roots: [] as string[],
...
vi.mock('../specialAgentConfig.js', () => ({
  resolveSpecialAgentSkillRoots: () => mockResolveSpecialAgentSkillRoots(),
}))
...
describe('cmdSync', () => {
  ...
  it('prints labeled roots when special-agent roots are detected', async () => {
    interactive = false
    mockResolveSpecialAgentSkillRoots.mockResolvedValueOnce({
      roots: ['/auto'],
  ...
  it('skips telemetry when SKILLHUB_DISABLE_TELEMETRY is set', async () => {
    interactive = false
    process.env.SKILLHUB_DISABLE_TELEMETRY = '1'
    mockApiRequest.mockImplementation(async (_registry: string, args: { path: string }) => {
      if (args.path === '/api/v1/whoami') return { user: { handle: 'steipete' } }
      if (args.path.startsWith('/api/v1/resolve?')) {
        return { match: { version: '1.0.0' }, latestVersion: { version: '1.0.0' } }
    ...
    delete process.env.SKILLHUB_DISABLE_TELEMETRY
  })
})
