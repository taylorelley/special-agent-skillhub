...
describe('registry resolution', () => {
  it('prefers explicit registry over discovery/cache', async () => {
    readGlobalConfig.mockResolvedValue({ registry: 'https://auth.skillhub.com' })
    discoverRegistryFromSite.mockResolvedValue({ apiBase: 'https://skillhub.ai' })
  ...
  it('ignores legacy registry and updates cache from discovery', async () => {
    readGlobalConfig.mockResolvedValue({ registry: 'https://auth.skillhub.com', token: 'tkn' })
    discoverRegistryFromSite.mockResolvedValue({ apiBase: 'https://skillhub.ai' })

    const registry = await getRegistry(makeOpts(), { cache: true })

    expect(registry).toBe('https://skillhub.ai')
