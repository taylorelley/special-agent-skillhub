...
import { resolveSpecialAgentDefaultWorkspace } from './cli/specialAgentConfig.js'
...
async function resolveWorkdir(explicit?: string) {
  ...
  const specialAgentWorkspace = await resolveSpecialAgentDefaultWorkspace()
  return specialAgentWorkspace ? resolve(specialAgentWorkspace) : cwd
}
...
async function hasSkillHubMarker(workdir: string) {
  ...
  const legacyLockfile = join(workdir, '.skillhub', 'lock.json')
  if (await pathExists(legacyLockfile)) return true
  const legacyMarkerDir = join(workdir, '.skillhub')
