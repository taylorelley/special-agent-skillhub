...
- CLI: `packages/skillhub/` (published as `skillhub`, legacy `skillhub`).
- Shared schemas/routes: `packages/schema/` (`skillhub-schema`).
...
- Persist install state:
  - `./.skillhub/lock.json` (per workdir, legacy `.skillhub`)
  - `./skills/<slug>/.skillhub/origin.json` (per skill folder, legacy `.skillhub`)
